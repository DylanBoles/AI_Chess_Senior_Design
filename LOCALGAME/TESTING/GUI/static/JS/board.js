    ...
    try {
        ...
        if (result.status === 'success' && result.engine_move) {
            ...
            if (result.game_over) {
                ...
                // In CPU vs CPU mode, automatically trigger next move (both sides are CPU)
                if (currentGameMode === "cpu_vs_cpu" && !isGamePaused) {
                    setTimeout(() => {
                        getEngineMove();
                    }, 1000);
                }
                // In user_vs_cpu mode, after engine move, it's user's turn again
                else if (currentGameMode === "user_vs_cpu" && !isGamePaused) {
                    document.getElementById('click-status').textContent = 
